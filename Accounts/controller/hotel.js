...
async function getHotelById(req, res) {
    try {
        let hotel = await Hotel.findById(req.params.userId);
        if (hotel) {
...
async function changeHotelInfo(req, res) {
    ...
    if (result.success) {
        ...
        if (
            req.body.email && req.body.hotelName && req.body.hotelDescription
            && req.body.city && req.body.address && req.body.phone && req.body.rooms && req.body.pricePerNight
        ) {
            // try {
...
module.exports = { registerHotel, loginHotel, getHotels, getHotelById, changeHotelInfo, deleteHotel }